        ...
        private (string Title, (string Heading, string Body)[] Sections) GetCurrentPageRules()
        {
            if (CharacterDetailPage.Visibility == Visibility.Visible)
            {
                ...
                [
                    ...
                    ("血量和 Stage", "使用 NumberBox，只保存有效正整数；Stage 对应 FantasyProject 的 FCharacterData.Stage，表示角色形态阶段，不是回合阶段。"),
            ...
            return _viewModel.SelectedModule switch
            {
                ...
                [
                    ...
                    ("导入与导出", "跨电脑交换角色必须使用顶部“导入/导出”：点击导出后进入卡片选择模式，直接在角色卡上勾选，也可全选当前结果；确认后导出统一 zip 包，不要手动覆盖 Characters。"),
                ...
                [
                    ...
                    ("填写字段", "手牌资料包含名称、英文代号、介绍、花色、扑克数字、卡牌类型、函数组、剩余使用次数、装备类型、数值和共鸣牌表达式。"),
                    ("装备数值", "灵装是之后给玩家看的装备大类包装，内部装备类型仍按武器、防具、道具管理。EXValue 中武器表示攻击范围，防具表示固定减伤，不是消耗型护盾。"),
                    ("剩余使用次数", "代表当前手牌的可用次数；-1 代表无限制。"),
                    ("导入与导出", "跨电脑交换手牌必须使用顶部“导入/导出”：点击导出后进入卡片选择模式，直接在手牌卡上勾选，也可全选当前结果；仍可导入 zip 或从其他项目导入，不要手动覆盖 HandCards。"),
                ...
                [
                    ...
                    ("开发者发布中心", "只在检测到源码工程时显示；可填写更高版本号、打包正式版或测试版、发布最新包，并查看 GitHub、Gitee 和 Bandizip 配置。"),
